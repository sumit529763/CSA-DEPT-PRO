...
const uploadFromBuffer = (buffer) => {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { folder: "news", quality: "auto", fetch_format: "auto" },
      (error, result) => {
...
exports.createNews = async (req, res) => {
  try {
    ...
    // ✨ AUDIT LOG
    await logAction(req.user._id, "CREATE_NEWS", `Posted News: ${title}`, req);
...
exports.updateNews = async (req, res) => {
  try {
    ...
    // ✨ AUDIT LOG
    await logAction(req.user._id, "UPDATE_NEWS", `Updated News: ${updatedNews.title}`, req);
...
exports.deleteNews = async (req, res) => {
  try {
    ...
    // ✨ AUDIT LOG
    await logAction(req.user._id, "DELETE_NEWS", `Deleted News: ${newsTitle}`, req);
...
// Public Methods (No logging needed)
