...
export default function ManageExam() {
  ...
  const emptyForm = {
    id: null, category: "Schedules", title: "",
    examDate: "", code: "", releaseDate: "",
    resourceType: "PDF", fileUrl: "", isPublished: true,
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.category === "Schedules") {
      data.append("code", formData.code);
      if (formData.examDate) data.append("examDate", formData.examDate);
    }
    if (formData.category === "Results") {
      if (formData.releaseDate) data.append("releaseDate", formData.releaseDate);
    }
    if (formData.category === "Resources") {
  ...
  const handleOpenEdit = (item) => {
    setIsEditMode(true);
    setFormData({
      id: item._id, category: item.category, title: item.title,
    ...
    <div className="exam-page">
      ...
      <div className="exam-header">
        <div>
          ...
          <p>Manage exam schedules, results and resources</p>
        </div>
        <button className="exam-add-btn" onClick={handleOpenAdd}>
          <i className="fas fa-plus" />
          <span>Add {activeTabMeta.singular}</span>
        </button>
      </div>
      ...
      <div className="exam-tabs">
        {TABS.map(({ key, icon }) => (
          <button
            key={key}
            className={`exam-tab ${activeTab === key ? "active" : ""}`}
            onClick={() => { setActiveTab(key); setSearch(""); }}
          >
            <i className={`fas ${icon}`} />
            <span>{key}</span>
            <span className="exam-tab-count">
              {allData.filter((d) => d.category === key).length}
            </span>
          </button>
        ))}
      </div>
      ...
      <div className="exam-toolbar">
        <div className="exam-search">
          ...
          />
          {search && (
            <button className="exam-search-clear" onClick={() => setSearch("")}>✕</button>
          )}
        </div>
        {!loading && (
          <p className="exam-count">
            <strong>{filtered.length}</strong> {activeTab.toLowerCase()}
      ...
      {loading ? (
        ...
        <div className="exam-empty">
          ...
          <p>Click "Add {activeTabMeta.singular}" to create an entry</p>
        </div>
      ) : (
        <>
          {/* ── DESKTOP TABLE ── */}
              ...
              <tbody>
                {filtered.map((item, idx) => (
                  <tr key={item._id}>
                    ...
                    {activeTab === "Schedules" && (
                      <>
                        <td><span className="exam-code">{item.code || "—"}</span></td>
                        <td className="exam-date">{formatDate(item.examDate)}</td>
                      </>
                    )}
                    {activeTab === "Results" && (
                      <td className="exam-date">{formatDate(item.releaseDate)}</td>
                    )}
                    {activeTab === "Resources" && (
                      <td><span className="exam-res-type">{item.resourceType || "—"}</span></td>
                    )}

                    <td>
                      <button
                        className={`exam-toggle ${item.isPublished ? "published" : "draft"}`}
                    ...
                    <td>
                      {item.fileUrl ? (
                        <a href={item.fileUrl} target="_blank" rel="noreferrer" className="exam-file-link">
                          <i className="fas fa-file-alt" /> View
                        </a>
                      ) : (
                        <span className="exam-no-file">—</span>
                      )}
                    </td>
                    ...
                    <td>
                      <div className="exam-actions">
                        ...
                        <button className="exam-btn-delete" onClick={() => handleDelete(item._id, item.title)}>
              ...
              <div className="exam-card" key={item._id}>
                <div className="ec-top">
                  <div className="ec-meta">
                    ...
                    {activeTab === "Schedules" && item.code && (
                      <span className="exam-code">{item.code}</span>
                    )}
                    {activeTab === "Resources" && (
                      <span className="exam-res-type">{item.resourceType}</span>
                    )}
                  </div>
                  <button
                    className={`exam-toggle ${item.isPublished ? "published" : "draft"}`}
                ...
                {activeTab === "Schedules" && item.examDate && (
                  <p className="ec-date">
                    <i className="fas fa-calendar-alt" /> {formatDate(item.examDate)}
                  </p>
                )}
                {activeTab === "Results" && item.releaseDate && (
                  <p className="ec-date">
                    <i className="fas fa-clock" /> Released: {formatDate(item.releaseDate)}
                  </p>
                )}
                ...
                <div className="ec-bottom">
                  {item.fileUrl ? (
                    <a href={item.fileUrl} target="_blank" rel="noreferrer" className="exam-file-link">
                      <i className="fas fa-file-alt" /> View File
                    </a>
                  ) : (
                    <span className="exam-no-file">No file</span>
                  )}
                  <div className="exam-actions">
                    <button className="exam-btn-edit"   onClick={() => handleOpenEdit(item)}>
